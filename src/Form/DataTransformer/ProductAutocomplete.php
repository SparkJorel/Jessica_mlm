...
class ProductAutocomplete implements DataTransformerInterface
{
    /**
     * @var ObjectManager
     */
    ...
    {
        if (null === $value || !$value instanceof Product) {
            return '';
        }
        ...
        $price = $repository->findPriceProduct($value->getCode());

        if (null === $price) {
            return '';
        }

        return $price->getProduct()->getCode() . ' ('. $price->getPrice() .')';
    }
    ...
    public function reverseTransform($value)
    {
        ...
        if (null === $product) {
            throw new TransformationFailedException(sprintf(
                'There is no "%s" exists in our product',
                $value
            ));
        }
        return $product;
